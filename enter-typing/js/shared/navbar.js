...
(function () {
  ...
  // 로그인 토큰은 서버가 HttpOnly 쿠키(ep_session)로 관리한다. 여기에는 화면 표시용 정보만 저장한다.
  ...
  function clearUser() {
    ...
    localStorage.removeItem('ep_token'); // 구버전이 남긴 토큰 정리
  }

  function logout() {
    // 쿠키는 JS 로 지울 수 없으므로 서버에 로그아웃을 요청한다.
  ...
  // 화면에는 로그인 상태인데 서버 세션(쿠키)이 만료/없는 경우를 정리한다.
  function verifySession() {
    if (!getUser()) return;
    fetch('/api/me').then(function (res) {
      if (res.status === 401) { clearUser(); location.reload(); }
    }).catch(function () {});
  }
