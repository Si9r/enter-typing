...
import models
from core.config import REDIS_ROOM_TTL
from core.redis_client import get_redis
from core.security import SESSION_COOKIE, get_current_user, get_password_hash, get_user_from_token, verify_password
...
LOBBY_MAX_CONNECTIONS = 500
LOBBY_MAX_PER_IP = 5
_lobby_ip_counts: Dict[str, int] = {}
...
async def create_battle_room(req: BattleRoomCreate, current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def select_room_content(room_code: str, req: RoomContentSelect, current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    redis = await get_redis()
...
async def lobby_websocket(websocket: WebSocket):
    # 인증 없이 열려 있으므로 전체/IP별 연결 수를 제한한다.
    ip = websocket.client.host if websocket.client else "unknown"
    forwarded = websocket.headers.get("x-forwarded-for")
    if forwarded:
        ip = forwarded.split(",")[-1].strip()
    if lobby_manager.get_connections("lobby") >= LOBBY_MAX_CONNECTIONS or _lobby_ip_counts.get(ip, 0) >= LOBBY_MAX_PER_IP:
        await websocket.close(code=1013)
        return
    _lobby_ip_counts[ip] = _lobby_ip_counts.get(ip, 0) + 1
    await lobby_manager.connect("lobby", websocket, "guest")
    try:
        while True:
            # 클라이언트 메시지는 사용하지 않는다. 비정상적으로 큰 메시지가 오면 끊는다.
        ...
        _lobby_ip_counts[ip] = max(0, _lobby_ip_counts.get(ip, 1) - 1)
        if not _lobby_ip_counts[ip]:
            _lobby_ip_counts.pop(ip, None)
...
async def battle_websocket(
    ...
    # 0. 다른 사이트에서 로그인 쿠키를 실어 웹소켓을 여는 것을 막는다.
    origin = websocket.headers.get("origin")
    if origin and urlparse(origin).netloc != websocket.headers.get("host"):
        await websocket.close(code=4003)
        return

    # 1. 로그인 쿠키(HttpOnly)의 JWT 검증. 토큰을 URL 쿼리로 받지 않는다 (로그/기록에 남지 않도록).
